...
// This store for loaded posts for user and split loaded posts for user and for the internal store
const userInfo = {
...
export const getStreamPage = ({
  ...
  // For load new posts/comments/reposts/likes from loaded sources every one minute
  if (init) {
...
/**
 * This funtion set current user status for detect user is active ?
 * if yes then stop update currecnt loaded stream
 * @param {userStatuses} status
 */
...
/**
 * This function add new post to our internal store and build new stream with new post and return new buit stream
 * @param { post, sources }
 * @returns { lengthOfUserRootPosts, lengthOfInternalRootPosts, stream }
 */
...
/**
 * This function get all parrent posts if post have type "reply" and return tree with rootPost and them comments
 * @param { postHashToComment, subscribedSources }
 * @returns { rootPost, replies, parentPosts }
 */
...
/**
 * This function get all replies by current root post and return an array of replies to root post
 * @param { postHashToComment, subscribedSources }
 * @returns replies[]
 */
...
/**
 * This function get post by hash from the internal store and return object with root post
 * with them replies + detect clicked post
 * @param { hash, subscribedSources }
 * @returns { clickedPost, rootPost, replies }
 */
...
/**
 * This function get all sources from the internal store and return
 * @returns { internalStore.sourcesByAddress }
 */
...
/**
 * This function get source by address from the internal store and return found source
